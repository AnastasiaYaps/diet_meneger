...
#pragma endregion
	private: System::Void button1_Click(System::Object^ sender, System::EventArgs^ e) {
		...
		if (product1 == "" || kkal == "") {
			...
			while (dbReader->Read()) {
				...
				if (product_check == product1) {
					MessageBox::Show("Продукт" + product1 + " уже есть в базе!", "", MessageBoxButtons::OK);
					fl = 1;
					dbReader->Close();
					dbConnection->Close();
					break;
		...
		if (fl == 0) {
			n++;
			String^ query1 = "INSERT INTO [Таблица1] (Продукт,калорийность )VALUES ('" + product1 + "' , '" + kkal + "')";
			OleDbCommand^ dbCommand1 = gcnew OleDbCommand(query1, dbConnection);
			dbCommand1->ExecuteNonQuery();
			MessageBox::Show("Новый продукт " + product1 + " добавлен!", "", MessageBoxButtons::OK);
			dbReader->Close();
			dbConnection->Close();
			this->Close();
		}}
